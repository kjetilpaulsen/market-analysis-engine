...
from market_analysis_engine.runtime.runtime import AppPaths, CFGLogging
from market_analysis_engine.utils.logging import setuplogging
...
def test_setup_logging_calls_configure_and_sets_initialized(monkeypatch, tmp_path: Path):
    ...
    setuplogging.setup_logging("market_analysis_engine", paths, log_cfg)

    assert captured["args"] == ("market_analysis_engine", paths, log_cfg)
...
def test_ensure_setup_logging_only_runs_once(monkeypatch, tmp_path: Path):
    ...
    setuplogging.ensure_setup_logging("market_analysis_engine", paths, log_cfg)
    setuplogging.ensure_setup_logging("market_analysis_engine", paths, log_cfg)

    assert calls == [("market_analysis_engine", paths, log_cfg)]
    assert setuplogging._LOGGING_INITIALIZED is True
...
def test_configure_logging_adds_stderr_handler_only(tmp_path: Path):
    ...
    try:
        setuplogging._configure_logging("market_analysis_engine", paths, log_cfg)
