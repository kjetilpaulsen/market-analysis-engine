...
from market_analysis_engine.runtime.runtime import CFGDev, CFGTickerService
...
class TickerService:
    ...
    def update(self, fullupdate=False) -> list[Ticker]:
        ...
        if self._cfg_dev.dev_mode:
            self._ticker_list = ["AAPL", "GOOGL", "DIS", "NVDA", "CAT", "META", "TSLA"]
        else:
            self._ticker_list = self._ticker_list
        
        logger.info(f"Retrieved updated list of tickers(dev={self._cfg_dev.dev_mode})")
        ...
        if fullupdate:
            logger.info("fullupdate True: Resetting all dates to None")
            for key, val in self._ticker_dict.items():
